...
def get_teacher_schedule(teacher_id: int, db: Session = Depends(get_db)):
    """Ручка для получения расписания преподавателя"""
    schedules = db.query(Schedule).filter(Schedule.prepod_id == teacher_id).all()
    return schedules
...
def get_student_schedule(student_id: int, db: Session = Depends(get_db)):
    """Ручка для получения расписания студента"""
    # 1. Сначала находим, в какой группе учится студент
    student_group = db.query(StudentGroup).filter(StudentGroup.student_id == student_id).first()
    
    if not student_group:
        raise HTTPException(status_code=404, detail="Студент не привязан ни к одной группе")
        
    # 2. Ищем расписание по найденному group_id
    schedules = db.query(Schedule).filter(Schedule.group_id == student_group.group_id).all()
    return schedules
